...
mermaid: true
math: true
last_modified_at: 2024-07-25 20:00:00 +0900
...
```
##### 2. Mockito Extension 설정

Mockito를 JUnit 5와 함께 사용하려면 `@ExtendWith(MockitoExtension.class)` 어노테이션을 사용하여 Mockito의 확장을 활성화해야 합니다.
...
##### 4. Mockito 메소드
테스트 메서드에서 `given`을 사용하여 Mockito의 동작을 설정하고, MockMvc를 사용하여 HTTP 요청을 시뮬레이션하는 코드입니다.
...
`verify` 메소드를 사용하여 메소드가 호출되었는지 검증하고 `ArgumentCaptor`를 사용하여 메소드 호출 시 전달된 인수를 캡처할 수 있습니다. 밑은 또 다른 예시 코드입니다.

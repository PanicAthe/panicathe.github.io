...
mermaid: true
math: true
last_modified_at: 2024-07-08 23:00:00 +0900
---
<style>
/* 큰제목 스타일 */
h2 {
...
#### 페이지 폴트
페이지 폴트는 프로세스가 접근하려는 메모리 페이지가 현재 물리적 메모리에 없는 경우 발생하는 이벤트.
...
### 인터럽트
인터럽트는 CPU의 현재 작업을 중단하고, 사전에 정의된 인터럽트 처리 루틴(Handler)을 실행하도록 하는 신호입니다.
- **종류**:
  - **하드웨어 인터럽트**: 외부 장치(예: 키보드, 마우스, 타이머 등)가 발생시키는 인터럽트입니다. 예를 들어, 키보드 입력이 있을 때 발생합니다.
  - **소프트웨어 인터럽트**: 소프트웨어가 명령어를 통해 발생시키는 인터럽트입니다. 예를 들어, 시스템 호출 또는 예외(잘못된 명령어 실행 시) 등이 있습니다.
- **작동 원리**:
  - 인터럽트가 발생하면 CPU는 현재 실행 중인 작업을 중단하고, 인터럽트 벡터 테이블을 참조하여 해당 인터럽트의 처리 루틴 주소를 찾습니다.
  - 인터럽트 처리 루틴을 실행하여 필요한 작업을 수행한 후, 중단된 작업을 재개합니다.
...
- 프로세스의 가상 메모리에 페이지 테이블이 직접 포함되는 것은 아니지만, 페이지 테이블을 가리키는 포인터(예: CR3 레지스터)가 PCB에 저장
